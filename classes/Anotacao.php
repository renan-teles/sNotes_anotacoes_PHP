...
class Anotacao extends Conexao {
    ...
    public function criarAnotacao($id_usuario, $titulo, $anotacao, $dataCriada, $horaCriada){
        try {
            $sql = 'INSERT INTO anotacoes (id_usuario, titulo, anotacao, dataCriada, horaCriada, naLixeira) 
                    VALUES (:idUser, :titulo, :anotacao, :dataCriada, :horaCriada, :status)';
            $result = $this->conexao->prepare($sql);
            $result->bindParam(':idUser', $id_usuario, PDO::PARAM_INT);
            $result->bindParam(':titulo', $titulo, PDO::PARAM_STR);
            $result->bindParam(':anotacao', $anotacao, PDO::PARAM_STR);
            $result->bindParam(':dataCriada', $dataCriada, PDO::PARAM_STR);
            $result->bindParam(':horaCriada', $horaCriada, PDO::PARAM_STR);
            $result->bindValue(':status', 'n', PDO::PARAM_STR);
            $result->execute();
        } catch (PDOException $e) {
            echo "Erro ao criar anotação: " . $e->getMessage();
        }
    }

    public function editarAnotacao($id, $campo, $conteudo){
        $camposPermitidos = ['titulo', 'anotacao', 'naLixeira']; 
        ...
        try {
            ...
            echo "Erro ao editar anotação: " . $e->getMessage();
        }
    }
    ...
    public function deletarAnotacao($id){
        try {
            ...
            echo "Erro ao deletar anotação: " . $e->getMessage();
        }
    }
    ...
    public function mostrarAnotacoes($idUsuario){
        try {
            ...
            echo "Erro ao mostrar anotações: " . $e->getMessage();
        }
    }
    ...
    public function mostrarAnotacoesLixeira($idUsuario){
        try {
            ...
            echo "Erro ao mostrar anotações na Lixeira: " . $e->getMessage();
        }
    }
    ...
    public function preecherAtributosObjeto($idAnotacao){
        try {
            ...
            $result = $this->conexao->prepare($sql);
            $result->bindParam(':id', $idAnotacao, PDO::PARAM_INT);
            $result->execute();
            ...
            echo "Erro ao preencher atributos: " . $e->getMessage();
        }
    }
}
?>
